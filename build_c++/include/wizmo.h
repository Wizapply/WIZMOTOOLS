...
typedef struct _simvr_data_packet
{
	...
	float pivotX;							//MASSIVE 軸ピボットX(mm)
	float pivotY;							//MASSIVE 軸ピボットY(mm)
	float pivotZ;							//MASSIVE 軸ピボットZ(mm)
...
private:
	void Update(WIZMODataPacket& packet);		// USB 経由
	void UpdateLAN(WIZMODataPacket& packet);	// LAN 経由
	void ThreadUpdate();
